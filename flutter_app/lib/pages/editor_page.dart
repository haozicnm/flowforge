...
class _EditorPageState extends State<EditorPage> {
  ...
  Future<void> _execute() async {
    if (_workflow == null) return;
    setState(() { _isExecuting = true; _output = '执行中...'; _failedNodes.clear(); _nodeDurations.clear(); _nodeOutputs.clear(); });
    try {
      await _save();
      ...
      if (result.isSuccess) {
        ...
        setState(() {
          ...
          result.nodeOutputs.forEach((nodeId, outputs) {
            if (outputs is Map) _nodeOutputs[nodeId] = Map<String, dynamic>.from(outputs);
          });
        });
      } else {
        buf.writeln('❌ 执行失败');
  ...
  Future<void> _step() async {
    ...
    try {
      ...
      final result = await widget.api.executeStep(
        _workflow!.id,
        completed: _stepCompleted,
        failed: _stepFailed,
        nodeOutputs: _nodeOutputs,
      );
      final buf = StringBuffer();
      if (result.status == 'stepped') {
        buf.writeln('⏭ 执行了: ${result.executed.join(", ")}');
        buf.writeln('已完成: ${result.completed.length} 个节点');
        if (result.hasMore) {
    ...
    return KeyboardListener(
      ...
      onKeyEvent: (e) {
        if (e is KeyDownEvent) {
          if (e.logicalKey == LogicalKeyboardKey.f10) _step();
        }
      },
      child: Padding(
  ...
  void _addNodeFromPalette(NodeTypeDef t) {
    ...
    final node = WorkflowNode(
      ...
      label: t.displayName,
      config: {},
      position: {'x': 100 + _nodes.length * 30.0, 'y': 100 + _nodes.length * 20.0},
    );
    setState(() {
      _nodes.add(node);
      _selectedNodeId = id;
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: CanvasEditor(
          ...
          nodeOutputs: _nodeOutputs,
          groups: _groups,
          onGroupsChanged: (groups) => setState(() { _groups.clear(); _groups.addAll(groups); }),
        ),
      ),
    );
  }
  ...
  Widget _buildTopBar(ThemeData theme, FlowForgeThemeExtension ext) {
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        FfButton(
          onTap: _isExecuting ? null : _step,
          builder: (ctx, hovering) => Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                FfSvg(FfIconName.play, size: 16, color: ext.brandColor),
                const SizedBox(width: 4),
                FfText('单步(F10)', fontSize: 12),
              ],
            ),
          ),
        ),
        const SizedBox(width: 8),
        FfButton(
          onTap: _isExecuting ? null : () { _resetStepState(); _execute(); },
          builder: (ctx, hovering) => Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            child: Row(
              mainAxisSize: MainAxisSize.min,
  ...
  IconData _nodeIcon(String type) {
    switch (type) {
      ...
      case 'try_catch': return Icons.error_outline;
      case 'variable': return Icons.data_object;
      case 'json': return Icons.data_array;
      case 'regex': return Icons.find_replace;
      case 'template': return Icons.description;
      case 'webhook': return Icons.webhook;
      case 'email_send': case 'email_read': return Icons.email;
      case 'ftp_upload': case 'ftp_download': return Icons.cloud_upload;
      case 'image_process': return Icons.image;
      case 'excel_read': case 'excel_write': return Icons.table_chart;
      case 'docx_read': case 'docx_create': return Icons.article;
      case 'pdf_extract': return Icons.picture_as_pdf;
  ...
  Color _nodeColor(String type) {
    switch (type) {
      ...
      case 'shell': case 'script': return Colors.red;
      case 'condition': case 'loop': case 'try_catch': return Colors.purple;
      case 'variable': case 'json': case 'regex': case 'template': return Colors.teal;
      case 'webhook': return Colors.indigo;
      case 'email_send': case 'email_read': return Colors.amber;
      case 'ftp_upload': case 'ftp_download': return Colors.cyan;
      case 'image_process': return Colors.pink;
      case 'excel_read': case 'excel_write': return Colors.lightGreen;
      case 'docx_read': case 'docx_create': return Colors.blueGrey;
      case 'pdf_extract': return Colors.brown;
