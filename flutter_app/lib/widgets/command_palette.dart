...
class _CommandPaletteState extends State<CommandPalette> {
  ...
  void _execute(PaletteCommand cmd) {
    if (cmd.id == 'page-dashboard') {
      // Navigate to dashboard — handled by parent
    } else if (cmd.id == 'page-settings') {
      widget.onNavigateSettings?.call();
    } else if (cmd.id.startsWith('wf-')) {
      final wfId = cmd.id.substring(3);
      widget.onOpenWorkflow?.call(wfId);
    } else if (cmd.id.startsWith('node-')) {
      // Add node — communicate with editor page via a different channel
      // For now just close the palette
    }
    // Close palette by removing from overlay
    Overlay.of(context)..let((o) {
      // Workaround: close is handled by the toggle function
    });
  }
